 ...
 *      }
 */
export default function CevaTemplate({ shipment, airline, warehouse, permit = {} }) {
  ...
  const fromFacility = permit.fromFacility || (() => {
  ...
  const toFacility = permit.toFacility ||
    (destWarehouse
      ? {
  ...
  const fmtNum = (n) => (n === null || n === undefined || n === '' ? '' : Math.round(n).toLocaleString('en-US'));
  let weightDisplay = '';
  ...
  const arrivalAirline =
    permit.arrivalAirline || (airline?.name ? `${airline.name}${airline?.code ? ` (${airline.code})` : ''}` : '');

  const flightNumber = permit.flightNumber || shipment?.flightNumber || '';
  const eta = permit.eta || shipment?.eta;
  ...
  const ourRef = permit.ourRef || '';
  const approvalDate = permit.approvalDate || '';

  const rows = permit.transferRows && permit.transferRows.length > 0 ? permit.transferRows : [{}, {}, {}];
    ...
    <div className={styles.page}>
      ...
      <div className={styles.headerRow}>
        ...
        <div className={styles.agencyBlock}>
          <div className={styles.agencyDept}>Department of Homeland Security</div>
          <div className={styles.agencyName}>U.S. Customs and Border Protection</div>
        </div>
      </div>
      ...
      <div className={styles.arrivalLine}>
        ...
        <span className={styles.fillSpan}>{eta ? `${formatDate(eta)} ${formatTime(eta)}` : ''}</span>
      </div>
      ...
      <div className={styles.manifestRow}>
        <div className={styles.manifestText}>
          As abstract of the vessels manifest covering the containers by B/L No., Marks, Numbers,
          Contents, Consignee, etc, is attached to each copy of this document.
        </div>
      ...
      <div className={styles.sigRow}>
        <div className={styles.sigBlock}>
          ...
          <div className={styles.sigLabel}>(Signature of Authorized Agent of Container Station)</div>
        </div>
        <div className={styles.sigBlock}>
          <div className={styles.sigLine} />
          <div className={styles.sigLabel}>(Signature of Agnt. Of Imp. Carrier)</div>
        </div>
      </div>
      ...
      <div className={styles.approvalRow}>
        <div className={styles.approvalLeft}>
          ...
          <div className={styles.approvalLabel}>(Signature of Customs Officer)</div>
        </div>
        <div className={styles.approvalRight}>
          <div className={styles.approvalDateValue}>{approvalDate ? formatDate(approvalDate) : '\u00A0'}</div>
          <div className={styles.approvalLine} />
